# verify_baseline.py
"""
Verify baseline fitting methods on healthy and degrading assets. Generates
control limit plots showing pre/post-failure detection capability.
"""
...
from analytics_config import (
    BASELINE_VALIDATION_DIR, BASELINE_WINDOW_HOURS, BASELINE_NUM_STD,
    ETL_PIPELINE_PATH, BASELINE_VALIDATION_SIGNAL_COLS, BASELINE_VALIDATION_NEEDED_COLS,
    FAILURE_TYPE_HEALTHY_VARIANTS,
)

from baseline import BaselineCalculator

os.makedirs(BASELINE_VALIDATION_DIR, exist_ok=True)
...
def debug_baseline_results(method_name, result_dict, signal, failure_idx, pre_failure_idx):
    """Print baseline statistics, violation counts, and pre/post-failure breakdown."""
...
def plot_baselines(asset_id, signal_col, data, failure_idx, suffix, is_healthy):
    """Generate baseline control limit plot with rolling, hourly, and state-based methods."""
    ...
    rolling = calc.apply_rolling(signal, num_std=BASELINE_NUM_STD)
    hourly = calc.apply_hourly(timestamps, signal, num_std=BASELINE_NUM_STD)
    state = calc.apply_state(flow, signal, num_std=BASELINE_NUM_STD)
    ...
    filename = os.path.join(BASELINE_VALIDATION_DIR, f"{asset_id}_{signal_col}{suffix}.html")
...
df = pd.read_sql_table("historian_clean", engine, columns=BASELINE_VALIDATION_NEEDED_COLS)
...
healthy_mask = df["failure_type"].isna() | df["failure_type"].isin(FAILURE_TYPE_HEALTHY_VARIANTS)
...
print(f"{'='*70}")
for signal_col in BASELINE_VALIDATION_SIGNAL_COLS:
    plot_baselines(healthy_asset, signal_col, df_healthy, failure_idx=None, suffix="_healthy", is_healthy=True)
...
print(f"{'='*70}")
print(f"Output plots saved to: {BASELINE_VALIDATION_DIR}")
